  ...
  sidebarLayout(
    sidebarPanel(
      ...
      selectInput("method_a", "Method", choices =
                  c("nlme population" = "nlme"),
                  c("Bayesian Stan" = "stan"),
                  selected = "nlme"),
      ...
      selectInput("model_a", "Curves analyzed as ", choices =
                  ...
                  selected = "linexp"),
      bsPopover("model_a",  "Available Models",
                pop_content["model_a"], "right"),
      conditionalPanel(
          ...
          selectInput("variant", "Variant",
                      c("1: v0 + tempt + kappa" = 1,
                        ...
                        "3: v0 + tempt" = 3)),
          bsPopover("variant",  "Available Variants",
                    pop_content["variant"])
      ), # conditionalPanel nlme
      conditionalPanel(
        condition = "input.method_a == 'stan'",
        selectInput("model_b", "Model name",
                    c("1" = 1,
                      "2" = 2,
                      "3" = 3)),
        bsPopover("model_b",  "Stan Model",
                  pop_content["stan_model"])
